...
if __name__ == "__main__":
    ...
    # random seed 에 따른 실험 진행
    for seed in CFG['seed']:
        # seed 설정
        pl.seed_everything(seed)
        
        # wandb 설정
        _train_name = "ENTbase/TypeCl"
        wandb_name = f"B:{CFG['train']['batch_size']}_LR:{CFG['train']['LR']['lr']}_M:{CFG['train']['token_max_len']}_{_train_name}_{CFG['train']['lossF']['name']}_seed{str(seed)}_E:{CFG['train']['epoch']}"
        wandb_logger = wandb.init(
            # name=folder_name,
            name = wandb_name,
            project="level2-experiment", entity=CFG['wandb']['id'], dir=save_path)
        ...
        # copyfile('latest-run_config.yaml','f"{save_path}/wandb/latest-run/files/config.yaml"')
